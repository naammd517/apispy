// Compact handling for ApiSpy.
const { EventEmitter } = require('node:events');

class CompactHandler extends EventEmitter {
 ...
 if (!payload) throw new Error('empty ' + 'compact' + ' payload');
 return { 'compact': String(payload).slice(0, 200) };
 }
...
module.exports = { CompactHandler, run: (payload, opts) => new CompactHandler().run(payload, opts) };